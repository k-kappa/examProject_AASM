...
########################code with gradienttape + frame stacking + grayscale
...
import procgen
numbers_lvls = 1
env = gym.wrappers.GrayScaleObservation(gym.make('procgen-maze-v0', 
                #render_mode="human",
                num_levels=numbers_lvls, start_level=15, 
...
model_Q.load_weights("./checkpoints_new/project_3.3_CNN_NEWPAPER_Model_maze_GRAYSCALE-gradienttape_100episodes_1lvl_rewSTANDARDeduration_quartoSINGOLOround_CLIPPING_0003LR.weights.h5")
...
lower_bound_epsilon = 0.3#.40  #prima era 20
###############################
title_save_weights = ("./checkpoints_new/project_3.3_CNN_NEWPAPER_Model_maze_GRAYSCALE-gradienttape_"
+str(number_episodes)+"episodes_"
+str(numbers_lvls)+"lvl_rewSTANDARDeduration_quintoSINGOLOround_CLIPPING_0003LR.weights.h5")
...
for episode in range(number_episodes): #for each episode...
    ...
    for s in range(max_number_steps): #for each step...
        ...
        if done:
            ...
            for x in temporary_buffer: 
                obs, action_value_new, index_to_update, _ , obs_new, action_value = x 
                new_x = obs, action_value_new, index_to_update, rewFinal , obs_new, action_value #backpropagate the final reward to all steps of the episode
    ...
    batch_size = 64                               #test with smaller natch size, before was 500
